...
class SliderCostume extends Module
{
	function __construct()
	{
		$this->name = 'slidercostume';
		$this->tab = 'slider';
		$this->version = '1.0';
		$this->author = 'Hetic';
	...
	{
	  	if (!parent::uninstall())
	    	Db::getInstance()->Execute('DELETE FROM `'._DB_PREFIX_.'slidercostume`');
	    parent::uninstall();
	}
		...
		for ( $i = 0; $i < 12; ++$i )
		{
			...
			if ( $_FILES[ $infile ]['error'] != UPLOAD_ERR_NO_FILE )
			{
				if ( isset($_FILES[ $infile ]['name']) && ($_FILES[ $infile ]['error'] == UPLOAD_ERR_OK) )
				{
					...
					{
						if ( move_uploaded_file( $_FILES[ $infile ]['tmp_name'], _PS_ROOT_DIR_.'/modules/slidercostume/images/'.$outfile ) )
						{
							$absImg = _PS_ROOT_DIR_.'/modules/slidercostume/images/'.$outfile;
...
/*###################################################################*/
	...
	function hooksliderCostume($params)
	{
	  	return $this->hookLeftColumn($params);
	}
}