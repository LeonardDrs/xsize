...
{else}
	...
	<ul class="block_various_links" id="block_various_links_footer">
		...
		{foreach from=$cmslinks item=cmslink key=numb}
			{if $cmslink.meta_title != ''}
				<li class="elemFoot2 {if $numb=='0_2'}first_item{else}item{/if}"><a href="{$cmslink.link|addslashes}" title="{$cmslink.meta_title|escape:'htmlall':'UTF-8'}">{$cmslink.meta_title|escape:'htmlall':'UTF-8'}</a></li>
			{/if}
		{/foreach}
