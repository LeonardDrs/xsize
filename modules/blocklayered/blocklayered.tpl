			...
			<div>
				...
				{foreach from=$filters item=filter}
					{if isset($filter.values)}
						...
						<span class="layered_close"><a class="layered_subtitle fleche_bas" href="#" rel="ul_layered_{$filter.type}_{$filter.id_key}">{$filter.name|escape:html:'UTF-8'}</a></span>
						<div class="clear"></div>
						...
						{else}
							{if $filter.filter_type == 0}
								<label for="{$filter.type}">{l s='Range:' mod='blocklayered'}</label> <span id="layered_{$filter.type}_blah"><span id="prix_min">{$filter.values[0]}{$filter.unit}</span><span id="prix_max">{$filter.values[1]}{$filter.unit}</span></span>
								...
								{literal}
									...
									addSlider('{/literal}{$filter.type}{literal}',{
										...
										slide: function( event, ui ) {
											stopAjaxQuery();
											$('#layered_{/literal}{$filter.type}{literal}_blah').html('<span id="prix_min">' + ui.values[ 0 ] + '{/literal}{$filter.unit}' + '</span>' + '{literal}' + '<span id="prix_max">' + ui.values[ 1 ] + '{/literal}{$filter.unit}'+ '</span>' + '{literal}');
										},
										stop: function () {
											reloadContent();
										}
...
<div id="layered_block_left" class="block">
	...
	<div id="layered_ajax_loader2" style="display: none;">
