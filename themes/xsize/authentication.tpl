		...
		<form action="{$link->getPageLink('authentication.php', true)}" method="post" id="create-account_form" class="std">
			<fieldset>
				...
				<p class="text">
					...
					<span class="mailspan"><input type="email" id="email_create" name="email_create" value="{if isset($smarty.post.email_create)}{$smarty.post.email_create|escape:'htmlall':'UTF-8'|stripslashes}{/if}" class="account_input" required/></span>
				</p>
				<p class="submit">
...
<form action="{$link->getPageLink('authentication.php', true)}" method="post" id="account-creation_form" class="std">
	...
	<fieldset  id="identityfield">
		...
		<p class="required text">
			<label class="labelidentity" for="customer_firstname">{l s='First name'}</label><sup>*</sup>
		...
		<p class="required text">
			<label class="labelidentity" for="customer_lastname">{l s='Last name'}</label><sup>*</sup>
		...
		<p class="required password">
			<label class="labelidentity" for="passwd">{l s='Password'}</label><sup>*</sup>
			<input type="password" class="text" name="passwd" id="passwd" />
			<span id="forminfo" class="form_info">{l s='(5 characters min.)'}</span>
		</p>
		<p class="select">
		...
		<p class="checkbox" >
			<input type="checkbox" name="newsletter" id="newsletter-identity" value="1" {if isset($smarty.post.newsletter) AND $smarty.post.newsletter == 1} checked="checked"{/if} />
			<label class="labelidentity" for="newsletter-identity">{l s='Sign up for our newsletter'}</label>
		</p>
		<!--<p class="checkbox" >
			<input type="checkbox"name="optin" id="optin" value="1" {if isset($smarty.post.optin) AND $smarty.post.optin == 1} checked="checked"{/if} />
			<label class="labelidentity" for="optin">{l s='Receive special offers from our partners'}</label>
		</p>-->
		{/if}
	</fieldset>
	<br/><br/>
	<fieldset class="account_creation" id="identityfield">
		...
		{foreach from=$dlv_all_fields item=field_name}
			...
			{elseif $field_name eq "postcode"}
				<p class="required postcode text">
					...
					<input type="text" class="text" name="postcode" id="postcode" value="{if isset($smarty.post.postcode)}{$smarty.post.postcode}{/if}" onkeyup="$('#postcode').val($('#postcode').val().toUpperCase());" />				</p>
		...
		<p class="required text" id="address_alias">
			<label class="labelidentity" for="alias">{l s='Assign an address title for future reference'} </label><sup>*</sup>
	...
	<fieldset class="account_creation dni">
		...
		<p class="required text">
			<label class="labelidentity" for="dni">{l s='Identification number'}</label><sup>*</sup>
			<input type="text" class="text" name="dni" id="dni" value="{if isset($smarty.post.dni)}{$smarty.post.dni}{/if}" />
			<span class="form_info">{l s='DNI / NIF / NIE'}</span>
		</p>
	</fieldset>
